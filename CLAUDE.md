...
Global workflow rules (feature workflow, design process, mandatory skills, code review checklist, conventions) live in `C:\Users\Miko\.claude\CLAUDE.md` and apply here automatically.
...
Canonical patterns live in `docs/standards.md` — read that file during any consistency audit. When running `consistency-audit`, point it at `docs/standards.md` first, then `src/components/` and `src/hooks/` for implementation reference.
...
## Warning: /clear destroys unsaved context
`/clear` wipes the entire conversation with no confirmation and triggers no hooks — requirements, decisions, and reasoning discussed but not yet written to a file are permanently lost. Before running `/clear`, manually invoke `journal` to save the session.
